-- Supabase schema for v3 cloud sync (fixed RLS)
...
DROP POLICY IF EXISTS "select_for_auth" ON public.families_state;
CREATE POLICY "select_for_auth" ON public.families_state
  FOR SELECT USING (auth.role() = 'authenticated');
DROP POLICY IF EXISTS "insert_for_auth" ON public.families_state;
CREATE POLICY "insert_for_auth" ON public.families_state
  FOR INSERT WITH CHECK (auth.role() = 'authenticated');
DROP POLICY IF EXISTS "update_for_auth" ON public.families_state;
CREATE POLICY "update_for_auth" ON public.families_state
  FOR UPDATE USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');
DROP POLICY IF EXISTS "delete_for_auth" ON public.families_state;
